...
const LeaveApplicationForm = () => {
  ...
  useEffect(()=>{
    const storage = localStorage.getItem('ResponseData')
    if(storage){
  ...
  useEffect(()=>{
    const addData = async ()=>{
      ...
      if(user){
        try{
          const res = await getEmployeeById(user.empId);
          if(res?.ok){
            ...
            setFormData((prev)=>{
              const data = {
                ...
                managerEmail:resData.manager.employeeEmail
              }
              return data
            })
            if(res.status === 403){
              toast.error('Cannot access try login again')
              navigate('/');
            }
          }else{
            toast.error('Session got expired')
            navigate('/');
          }
        }catch(err){
          console.log(err)
